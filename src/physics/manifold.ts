...
export class CollisionManifold {
    ...
    constructor(entityA: Entity, entityB: Entity, normal: Vec3, penetration: number) {
...
function getCollisionAxis(a: AABB, b: AABB): { normal: Vec3; penetration: number } | null {
...
function buildContactPoint(a: AABB, b: AABB, normal: Vec3, penetration: number): ContactPoint {
    const centerA = getAABBCenter(a);
    const centerB = getAABBCenter(b);
...
export function createAABBManifold(entityA: Entity, entityB: Entity, worldAABB: AABB, worldBABB: AABB): CollisionManifold | null {
